...
var TJobList = Class.create({
  ...
  processReceivedJobList: function(obj){
    ...
    $(this.wellcomeMessage).update('&nbsp;&nbsp;Wellcome '+this.userid);
  ...
  // updateTDInfo
  //----------------------------------
  updateTDInfo: function(td,info,elem){
    td.update('');
    if (info!=undefined) {
  ...
  refreshJobListTable: function(params){
    ...
    for ( var r=ini; r<fin; r++ ){
      ...
      if (this.jobList[r].status=='DONE') {
        a = new Element('a',{'href':"javascript:showRun('" +this.jobList[r].id + "',jobList);"});
        a.update(jname);
        td.update(a);
      ...
      // fname='';
      // if (this.jobList[r].qinfo!=undefined) {
      //   if (this.jobList[r].qinfo.fileName!='') {
      //     fname = this.jobList[r].qinfo.fileName;
      //   };
      //   if (this.jobList[r].qinfo.master!='NONE') {
      //     fname = fname+' / '+this.jobList[r].qinfo.master;
      //   };
      // };
      
      // nueva celda con fname 
      td = new Element('td',{'width':"16%"});
      this.updateTDInfo(td,this.jobList[r].qinfo,'fileName');
			row.appendChild(td);
		  
		  // nueva celda con master
      td = new Element('td',{'width':"10%"});
      this.updateTDInfo(td,this.jobList[r].qinfo,'master');
			row.appendChild(td);
      
		  // nueva celda con ftype
      td = new Element('td',{'width':"7%"});
      this.updateTDInfo(td,this.jobList[r].qinfo,'fileType');
			row.appendChild(td);
      
      // nueva celda con seq type
      td = new Element('td',{'width':"7%"});
      this.updateTDInfo(td,this.jobList[r].qinfo,'alphabet');
			row.appendChild(td);
      
      // nueva celda con a length
      td = new Element('td',{'width':"10%"});
      this.updateTDInfo(td,this.jobList[r].qinfo,'length');
			row.appendChild(td);
      
      // nueva celda con num seqs
      td = new Element('td',{'width':"5%"});
      this.updateTDInfo(td,this.jobList[r].qinfo,'numberOfSequences');
			row.appendChild(td);
			
			// nueva celda con result size
      td = new Element('td',{'width':"5%"});
      this.updateTDInfo(td,this.jobList[r].qinfo,'resultSize');
			row.appendChild(td);
    ...
    // era 3
		if ((!RUNNING) & (this.requestCount>3)) {
